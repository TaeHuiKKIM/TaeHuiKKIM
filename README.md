...
- **시장 심리와 차트 알고리즘** — GBM 랜덤워크 기반 **10종** 종목 시세에 뉴스 충격, 추세 지속, 개미털기, 점프 이벤트를 결합했습니다. 숏스퀴즈, 차익실현, 루머 적중/실패가 차트에 남기는 움직임을 게임 알고리즘으로 해석했습니다.
- **성장 루프** — 개미 **10티어** 진화 도감, **20종** 업적, 환생(프레스티지), **8시간·순자산 55% 이중 캡** 휴식 보상, 일일 출석.
- **수익 다변화** — 애널리스트 자동매매, 배당, 예금 복리, 부동산 임대, 장중 단타 미니게임.
- **엔지니어링** — 빌드리스 Vanilla JS + ES 모듈, PWA(서비스워커), **70개** Vitest 회귀 테스트, 무위험 복리 수렴을 막는 가격 클램프·오프라인 보상 캡·정보력 적중률 상한, 광고 SDK 교체 대비 `AdService`/`ShareService` 인터페이스 분리, 로컬 저장 + 세이브 코드.
