...
### 4. 개미 투자자 키우기 (Ant Investor Idle) — 방치형 주식 시뮬레이션 <sub>(창업 · 수익형 서비스 도전)</sub>

<img src="assets/portfolio/ant-live.png" alt="개미 투자자 키우기" width="300"/>

투자·금융 관심과 창업 목표를 담아 **토스 미니앱**을 겨냥해 만든 PWA 게임입니다.

- **시장 시뮬레이션** — GBM(기하 브라운 운동) 랜덤워크 기반 **10종** 종목 시세, 뉴스(호재/악재)·섹터 이벤트로 급등락, 정보력(찌라시 예고, 신뢰도 51~60%)·레버리지 메커닉.
- **성장 루프** — 개미 **10티어** 진화 도감, **20종** 업적, 환생(프레스티지), **8시간·순자산 40% 이중 캡** 오프라인 보상, 일일 출석.
- **수익 다변화** — 애널리스트 자동매매, 배당, 예금 복리, 부동산 임대, 장중 단타 미니게임.
- **엔지니어링** — 빌드리스 Vanilla JS + ES 모듈, PWA(서비스워커), **69개** Vitest 회귀 테스트, 광고 SDK 교체 대비 `AdService`/`ShareService` 인터페이스 분리, 로컬 저장 + 세이브 코드.
